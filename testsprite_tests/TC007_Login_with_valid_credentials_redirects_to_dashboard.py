...
async def run_test():
    ...
    try:
        ...
        # -> Type example@gmail.com into email (index 289), type password123 into password (index 298), then click the login button (index 299).
        ...
        # -> Fill the visible email (index 465) and password (index 473) fields with the valid credentials and click the login button (index 474) to attempt login again, then observe the page for redirect and Dashboard text.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[2]/div/form/div/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('example@gmail.com')
