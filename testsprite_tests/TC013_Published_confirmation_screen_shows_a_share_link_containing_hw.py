...
async def run_test():
    ...
    try:
        ...
        # -> Fill email and password fields and click the login button to submit credentials.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the '+ إنشاء واجب جديد' (Create new homework) button to open the homework creation flow.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div/a/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the first '📋 نسخ الرابط' (copy link) button for a homework to reveal the share link so it can be verified that the link contains '/hw/'.
        frame = context.pages[-1]
        # Click element
