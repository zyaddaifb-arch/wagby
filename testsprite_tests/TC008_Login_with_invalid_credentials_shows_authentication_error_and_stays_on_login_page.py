...
async def run_test():
    ...
    try:
        ...
        # -> Type 'nonexistent.user@example.com' into the email field (index 244), type 'WrongPassword!' into the password field (index 245), then click the 'تسجيل الدخول' submit button (index 248). After that, verify the page remains on /login and an error message containing 'Invalid' and 'password' is visible.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[2]/div/form/div/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('nonexistent.user@example.com')
