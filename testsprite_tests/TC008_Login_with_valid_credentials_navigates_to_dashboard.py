...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'تسجيل الدخول' (Login) button on the homepage to open the login page (prefer natural navigation over direct URL).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/nav/a/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Navigate to /login and load the login page (explicit navigate to http://localhost:3000/login). After navigation, verify URL contains '/login'.
        await page.goto("http://localhost:3000/login")
        
        # -> Type the teacher email into the email field (index 478) as the immediate action.
        frame = context.pages[-1]
        # Input text
